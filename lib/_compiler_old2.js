...
${loaded.map(n => store.icons[n].symbol).join('\n')}</svg>`;

    yield h(make());

    for (const [prefix, names] of Object.entries(collections)) {
        // let path = (`https://api.iconify.design/` +
        //             `${prefix}.js?icons=${names.join(',')}&pretty=1&callback=define_icon`);
        let path = (`https://api.iconify.design/` +
...
async function* make_viewporte({variables, global}) {
    const store = global.store;
    store.userAgent = Navigator.userAgent;
...
async function* make_favicon({variables, global}) {
    const {store, finished} = global.store;
